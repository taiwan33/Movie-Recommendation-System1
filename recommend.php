...
function similarity_distance($matrix,$person1,$person2)
{
    $similar=array();
    $sum=0;
    if(array_key_exists($person1,$matrix)){
...
function getRecommendation($matrix,$person)
{
    ...
    foreach($matrix as $otherPerson=>$value)
    {
        if($otherPerson!=$person)
        {
           ...
           {
            if (array_key_exists($person,$matrix) && !array_key_exists($key,$matrix[$person]))
            {
            ...
            if(!array_key_exists($key,$total))
                ...
                $total[$key]+=$matrix[$otherPerson][$key]*$sim;  //rating for same movie for other person
